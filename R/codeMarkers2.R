  ...
  if (!is.numeric(markersClean)) {
    ...
    if (refAll[1] == "minor") {
      ...
      maxAll <- unname(nchar(gsub(pattern = "[[:punct:]]",
                                  replacement = "", x = firstAlls[1])) %/% nchar(refAlls[1]))
    } else {
      refAlls <- refAll
      ## Get maximum number of alleles.
      maxAll <- nchar(markersClean[1, 1]) %/% nchar(refAll)
    }
    ...
    markersRecoded <- codeCharMarkers(markersClean, refAlls, 
                                      refAll[1] == "minor", maxAll, 7)
    #dimnames(markersRecoded) <- dimnames(markersClean)
