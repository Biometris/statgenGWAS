...
expect_true(any(grepl(pattern = "Genetic variance: 0.02059749", x = sumStg0)))
expect_true(any(grepl(pattern = "Residual variance: 1.854127", x = sumStg0)))
...
expect_true(any(grepl(pattern = "inflation-factor: 0.772", x = sumStg0)))

sumStg1 <- capture.output(summary(stg1))
expect_true(any(grepl(pattern = "Number of significant SNPs: 1", x = sumStg1)))
expect_true(any(grepl(pattern = "Smallest p-value among the significant SNPs: 0.5564107",
                      x = sumStg1)))
expect_true(any(grepl(pattern = "Largest p-value among the significant SNPs: 0.5564107",
                      x = sumStg1)))
expect_true(any(grepl(pattern = "LOD-score: 0.2546045", x = sumStg1)))
