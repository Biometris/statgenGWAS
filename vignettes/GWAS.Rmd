...
Suppose there are $n$ measurements of a phenotype for $t$ genotypes. Then a lineair mixed model for association mapping is typically expressed as
$$y = X\beta + Zu + e$$  
Here $y$ is a $n \times 1$ vector of observed phenotypes. $X$ is an $n \times q$ matrix of fixed effects, including mean, SNPs and other confounding variables. $\beta$ is a $q \times 1$ vector of coefficients of the fixed effects. $Z$ is the $n \times t$ incidence matrix mapping the observed phenotypes to the individuals. $u$ then is the random effect of the mixed model with $var(u) = \sigma^2_gK$, where $K$ is the kinship matrix derived from the marker information of the individual. Finally $e$ is an $n \times n$ matrix of residual effects, with $var(\epsilon) = \sigma^2_eI$. For this model the phenotypic variance-covariance matrix $Sigma$ can be represented as $Sigma = \sigma^2_gZKZ^{-1} + \sigma^2_eI$. 

$Sigma$ can then be used to obtain generalized least-square (GLS) effect size estimates and F-test P values for each SNP from the model.

## Application in the package

The single trait GWAS algorithm implemented in the statgenSSA package consists of two main steps. These steps are repeated for each combination of trial and trait for which the analysis is run.  

First the phenotypic variance-covariance matrix $Sigma$ is estimated. Two ways for doing this are included in the package, either using the Efficient Mixed Model Association (EMMA) algorithm [@Kang2008] or the Newton-Raphson (NR) algorithm [@Tunnicliffe1989]. 

The effect size estimates and F-test P values are obtained using the GLS algoritm proposed by @Segura2012.
...
* `imputeType = "beagle"`: Impute missing values using the independent beagle software [@Browning2016]. This software is included in the package and is called with its default settings. It requieres java to run. Since running beagle takes a long time for the current data, the call to beagle is only shown here but not run.    
...
The output of the *runSingleTraitGwas* function is an object of class GWAS. This is a list consisting of five components desribed below.  
...
Now there are significant SNPs for both traits. However since the traits are measured on a different scale, the effects for anthesis seem a lot larger than those for grain.yield. This scaling issue can be solved by setting `normalize = TRUE`.
...
Other parameters can be used to plot a subset of the chromosomes (`chr`) and directly export the plot to .pptx (`exportPptx = TRUE` and specify `pptxName`). Note that the officer and rvg packages are required for this. A full list of parameters can be found by running `help(plot.GWAS)`. 
...
In most cases using chromosome specific kinship matrices considerably improves the predicting power of the algorithm. In the package chromosome specific kinship matrices are calculated per chromosome using one of the methods described above, ignoring the information on the chromosome itself. E.g. the kinship matrix for chromosome one is computed from the markers that are not on chromosome one. Calling *runSingleTraitGwas* with `GLSMethod = "multi"` assures computation of chromosome specific kinship matrices.
...
The threshold for selecting significant SNPs in a GWAS analysis is computed by default using Bonferroni correction, with an alpha of 0.05. The alpha can be modified setting the option *alpha* when calling *runSingleTraitGwas*. Two other threshold types can be used as well, a fixed threshold (`thrType = "fixed"`) specifying *LODThr* for the value of the threshold, or a threshold that defines the n SNPs with the highest LOD score as significant SNPs. Set `thrType = "small"` together with `nSnpLOD = n` to do this.
...
Covariates can be included as extra fixed effects in the GWAS model. These covariates should be in to the gData object and can then be added to the model by specifying `covar` when running the GWAS analysis.  
...
By default all SNPs with a MAF lower than 0.01 are excluded from the analysis. This can be controlled by the parameter `MAF`. Setting MAF to 0 will still excluede duplicate SNPs since duplicates cause problems when fitting the underlying models. Instead of excluding SNPs with a low MAF it is also possible to exclude SNPs with a low minor allele count (MAC). This is done setting `useMAF = FALSE` and then specifying `MAC`.   
...
Apart from the significant SNPs, it could also be interesting to see the SNPs that are close to the significant SNPs. These can be included in the output by setting `sizeIncludedRegion` to a positive value. When doing so SNPs that are within a distance of `sizeIncludedRegion` from a significant SNP (based on the information in the map) are included in the signSnp data.table within the GWAS object. Only SNPs that are in a sufficient linkage disequilibrium (LD) with the significant SNP, measured in terms of $R^2$, are included. The minimum $R^2$ required to be included can be specified by setting `minR2`.
